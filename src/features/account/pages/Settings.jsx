...
const Toggle = ({ checked, onChange }) => (
  <button onClick={onChange}
    className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ${checked ? 'bg-secondary' : 'bg-gray-200'}`}>
            ...
            {['light', 'dark', 'system'].map(t => (
              <button key={t} onClick={() => setTheme(t)}
                className={`px-5 py-2 rounded-lg text-sm font-semibold border capitalize transition-all ${
                  theme === t ? 'bg-primary text-white border-primary' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-400'
                }`}>
                {t}
              </button>
            ))}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-red-100 overflow-hidden">
        ...
        <div className="divide-y divide-gray-100">
          <div className="px-5 py-4 flex items-center justify-between gap-4">
            ...
            <button className="px-4 py-2 text-sm font-semibold text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition-all shrink-0">
              Deactivate
            </button>
          </div>
          <div className="px-5 py-4 flex items-center justify-between gap-4">
