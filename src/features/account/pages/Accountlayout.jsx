          ...
          <aside className="hidden md:block w-64 flex-shrink-0 sticky top-24">
            <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
              ...
              <div className="flex items-center gap-3 mb-4 pb-4 border-b border-gray-100">
                <div className="w-12 h-12 rounded-full bg-[#131921] flex items-center justify-center text-white text-lg font-bold flex-shrink-0">
                  {user?.name?.[0] || 'U'}
                </div>
                <div className="min-w-0">
                  ...
                  <span className="inline-block text-[10px] bg-[#FF9F00]/15 text-[#b36b00] font-semibold px-2 py-0.5 rounded-full mt-1">
                    Gold Member
                  </span>
                </div>
              </div>
                ...
                {NAV.map(({ label, path, icon }) => {
                  ...
                  return (
                    <Link key={path} to={path}
                      className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all ${active ? 'bg-[#131921] text-white font-semibold shadow-md' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
                        }`}>
                      <span className={`material-symbols-outlined text-lg ${active ? 'text-[#FF9F00]' : 'text-gray-400'}`}>
                        {icon}
                      </span>
                      {label}
                    </Link>
          ...
          <main className="flex-1 min-w-0 w-full">
            ...
            <div className="flex md:hidden gap-2 overflow-x-auto no-scrollbar bg-white rounded-xl p-2 mb-4 shadow-sm border border-gray-100">
              {NAV.map(({ path, icon, mobileLabel }) => {
                ...
                return (
                  <Link key={path} to={path}
                    className={`flex flex-col items-center gap-0.5 px-4 py-2 rounded-lg text-[10px] font-bold flex-shrink-0 transition-all ${active ? 'bg-[#131921] text-white shadow-sm' : 'text-gray-500'
                      }`}>
                    <span className={`material-symbols-outlined text-base ${active ? 'text-[#FF9F00]' : ''}`}>{icon}</span>
                    {mobileLabel}
                  </Link>
                );
              })}
